...
function createStatement(user, ...movies) {
  const statementBuilder = [];
  statementBuilder.push(`Rental Record for ${user}`);

  movies.forEach((movie) => {
    const { price} = calculateRentalSummary(movie);
    statementBuilder.push(`  ${movie.name}  £${price.toFixed(1)}`);
  })

  const summary = calculateRentalSummary(...movies);

  statementBuilder.push(`You owe £${summary.price.toFixed(1)}`);
  statementBuilder.push(`You earned ${summary.points} frequent renter points`);
...
module.exports = { calculateRentalSummary, createStatement }
